...
每个商户收单场景至少要覆盖以下 A/B/C/D/E 五类用例,分别对应正向、异常、一致性、边界并发、权限安全。本页定义这套通用覆盖要求,是场景手册(§5.x)编写用例库时的最小覆盖面;具体场景的用例库见各场景手册,配合 [[merchant-acquisition-e2e-five-stage-verification]] 的五段式验证一起用。
...
错误码对照见 [[merchant-acquisition-error-code-quickref]] / [[merchant-acquisition-error-code-cheatsheet]],特殊 expiry 触发卡号见 [[merchant-acquisition-test-data-conventions]]。
...
详细的落库表清单与 Fund Flow 校验方法见 [[merchant-acquisition-e2e-five-stage-verification]],对账文件结构见 [[merchant-acquisition-notify-and-statement]] / [[merchant-acquisition-notify-statement-spec]]。
...
金额段与测试卡选择参见 [[merchant-acquisition-test-data-preparation]] / [[merchant-acquisition-test-data-conventions]]。
...
- 每个 §5.x 场景手册都按 A/B/C/D/E 五组扩展为具体用例库,按 A→E 顺序排列,确保每类至少 1 条
...
- 调用骨架可复用 [[merchant-acquisition-curl-postman-skeleton]] / [[merchant-acquisition-curl-postman-templates]]

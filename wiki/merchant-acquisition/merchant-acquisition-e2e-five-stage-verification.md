...
商户收单测试的核心方法论：一笔交易必须从接口响应一路验到对账单，缺一段就可能让"商户报已付款但订单未支付"、"对账不平"这类问题逃逸到生产。本页逐段拆解 5 个验证段的核对要点和高频踩坑。详细用例分组见 [[merchant-acquisition-test-case-skeleton-abcde]]。

## 总览：五段链路

```
① 接口响应 → ② 服务调用链 → ③ 数据库落库 → ④ Fund Flow → ⑤ 通知 & 对账
```

所有场景手册都按这 5 段验证，缺一不可。这套骨架来自实际线上事故复盘 —— 只测前几段不测后几段，错误才会逃逸。
...
- `head.code`（`0` 表示成功，错误见 [[merchant-acquisition-error-code-quickref]]）
...
**实操工具**：在 Kibana 按响应 head 里的 `traceCode` 全链路查；或登跳板机看应用 log。工具入口见 [[merchant-acquisition-test-environment-and-tools]]。

**典型核对清单（PreAuth 举例）**：

```
✓ acquireii log: paySceneCode=PREAUTH
✓ cashierii log: 收银台路由命中
✓ authPay log: 鉴权方案命中、风控通过
✓ cmf log: 选中渠道 MPGS101（按卡品牌+地域）
✓ router log: 路由到 qpay-mpgs
✓ qpay-mpgs log: apiOperation = 'AUTHORIZE'  ← 不是 PAY
✓ mpgs response: 成功
```
...
**建议固化**：每个场景手册都列出"该场景下应增加的具体表+条数"清单，跑

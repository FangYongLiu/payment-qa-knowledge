...
> 每跑一笔交易都按本清单逐项打 ✓，可直接贴到任务卡片或 PR 描述里。本清单是 [[merchant-acquisition-e2e-five-stage-verification]] 的浓缩版执行视图。
...
- [ ] `head.code = 0`
- [ ] `body` 中订单状态符合预期（`CREATED` / `PAID_SUCCESS` / `AUTHORIZATION` 等）
- [ ] 异常用例：错误码与场景对应（见 [[merchant-acquisition-error-code-quickref]]）
- [ ] 注意：`applyStatus=SUCCESS` ≠ 业务成功，必看 `code` 与 `status`
...
- [ ] 该产生入账流水的场景：`t_fund_flow` 有对应记录
- [ ] 该没有 Fund Flow 的场景（如 PreAuth 仅冻结 / Void / 下单未支付 / 支付失败）确认确实没有
...
- [ ] 重试用例：按 `2 → 10 → 10 → 60 → 120 → 360 → 900` 分钟节奏，第 7 次后停止
- [ ] T+1 对账单 ZIP 包含本笔（PreAuth 除外，仅 Capture 进对账单）
- [ ] CSV 中 `transactionType` / `status` / `direction` / 金额正确
- [ ] Settlement 等式：`totalCredit - totalDebit = settleToBank + stayAmount`

详见 [[merchant-acquisition-notify-and-statement]]。
...
- 测试数据/卡号/金额选择：[[merchant-acquisition-test-data-conventions]]
- 环境与工具：[[merchant-acquisition-test-environment-and-tools]]
- 调用骨架：[[merchant-acquisition-curl-postman-skeleton]]

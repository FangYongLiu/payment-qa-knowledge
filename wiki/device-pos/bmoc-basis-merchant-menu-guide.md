...
source_ref: wiki_image:ec9239d3-63bc-476e-8482-53632bed637c
tags: []
---
...
| 📌 设备管理 | TMS > Device Approval / Device ActiveCode / Activated Devices / Devices Channel Ma... / POS Settings / Amex Mapping / Operation Logs | 审批 POS、查看终端状态、激活码管理、通道映射等 |
...
BMOC 控台顶部为 `PAYBY BASIS` Logo 与 `WELCOME TO PAYBY` 欢迎语,右侧提供语言切换、外链、消息通知、全屏与当前登录用户信息(如 `Can Wang`)。

左侧主导航分为 `GENERAL`、`BUSINESS`、`TMS` 等一级模块,点击展开后进入对应子菜单。
...
## 核心菜单:设备审批(Device Approval)

- **路径**:`/TMS/Device Approval`
- **用途**:审批商户提交的 POS 设备申请,管理终端接入流程。
- **同级 TMS 子菜单**:`Device Approval`、`Device ActiveCode`、`Activated Devices`(含 `Activated Devices`、`Fiserv Outbound...`、`Fiserv Exception...` 等子项)、`Devices Channel Ma...`、`POS Settings`、`Amex Mapping`、`Operation Logs`。
- **筛选条件**:`Merchant ID`、`Merchant Name`、`Store Name`,以及状态下拉(默认 `ALL`),通过 `Search` 按钮检索。
- **列表字段**:`Apply ID`、`Merchant ID`、`Merchant Name`、`Contact Mobile`、`Store Name`、`Device Type`、`Device Number`、`Application Time`、`Status`、`Action`。
  - `Device Type` 常见取值:`CUSTOMIZED_`(如出租车定制设备)、`SMART_POS` 等。
  - `Status` 常见取值:`Approved`、`Rejected` 等。
  - `Action` 列提供 `View` 入口,可跳转申请详情页。
- **典型场景**:出租车 POS 替换流程中,在 `Device Approval` 中查找对应商户(如 `Cars Taxi L.L.C...`)的申请记录并跟进审核状态。

## 权限说明与账号管理建议

- 登录 BMOC 控台需使用员工域账号(如:`fangyong.liu`、`Can Wang`)。
- 系统角色与操作权限的配置归属 `Permissions` 菜单,操作记录通过 `Operate Log` 留痕;TMS 模块下另有 `Operation Logs` 记录设备相关操作。

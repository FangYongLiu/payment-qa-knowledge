...
本页汇总 PayBy 收单链路涉及的微服务应用、功能定位与对应负责人，便于排障、对接与责任溯源。完整业务背景见 [[payby-acquire-transaction-overview]]，相关数据库见 [[payby-acquire-databases-guide]]。
...
- **接入层**：gp028_sgs（网关）、gp071_merchant-frontend（SGS 前置）
...
- 各应用承载的支付场景与渠道编码见 [[payby-acquire-pay-scene-codes]]
- 各应用对应的核心数据库与常用表见 [[payby-acquire-databases-guide]]

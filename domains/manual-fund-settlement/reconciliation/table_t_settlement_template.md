...
related_scenarios:
- manual-fund-settlement-overview
...
## 用途
存储结算模板配置信息，位于 `reconciliation.t_settlement_template`。模板由清算团队在 counter — settlement management — settlement Template 创建，选择结算类型、填写出款账户信息后提交审核，审核通过后可用于发起结算申请。

## 关键列
- `settlementType`：结算类型，非配置，代码写死枚举。可选值包括：
  - `INNER_ACCOUNT`：内部账户结算，对应 biz_product_code 500105 / 500108 / 500109 / 500110 / 500111 / 500113
  - `MERCHANT_ACCOUNT`：商户出款，对应 biz_product_code 230601（AED to AED） / 230602（AED to USD） / CB001（CA 向 CA 内部账户转账） / CB103（CA 向 CA 以外账户转账）
- `settlement_code`：当为 CMA 账户间互转时需配置，例如 `CMA3->CMA4`。
...
- 模板需经审核通过后才能用于发起结算申请。
- payer / beneficiary / Bank Deposit Account 配置需与 `settlementType` 相匹配（如 MERCHANT_ACCOUNT 用 SettlementMerchantPayerInfoList，CB103 用对应 payer/beneficiary list）。

...
module: null
sensitivity: normal
name: ENBD余额监控定时任务(balanceMonitorJob)
aliases:
- balanceMonitorJob
related_services: []
related_tables: []
...
- 当 router 报 `ENBD 渠道 channel account balance is not enough` 时，可通过 counter 或 redis 确认余额查询是否正常，若余额正常可尝试重新发起出款（可能对方系统波动），详见 [[ts_enbd_channel_balance_not_enough]]

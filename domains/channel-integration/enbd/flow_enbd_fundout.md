...
## 概述
ENBD渠道出款端到端流程：fundout → cmf → router → ENBD银行。router 路由渠道时，会判断渠道 ext 是否配置了 payoutAccount，若配置则进行渠道余额校验，否则跳过校验后与 ENBD 银行交互。余额数据来源于 balanceMonitorJob 落地的 reconciliation 表与 redis 缓存。
...
- 渠道场景与账户：
  - 商户出款：ENBD201(INNER-ENBD卡)、ENBD202(DFT-非ENBD本地卡)、ENBD203(IFT-国际卡)，待清算账户 7843003004004000x（FundOut Pending for Clearing-CMA4-ENBD20x），银存账户 78410010170010004（Cash in Banks-ENBD CMA4）
  - 个人出款：ENBD204(INNER-ENBD卡)、ENBD205(DFT-非ENBD本地卡)，待清算账户 7843003004002000x（FundOut Pending for Clearing-CMA2-ENBD20x），银存账户 78410010170010002（Cash in Banks-ENBD CMA2）
  - 资金调拨：ENBD209(INNER-ENBD卡)、ENBD210(DFT-非ENBD本地卡)，待清算账户 7843003004003000x（FundOut Pending for Clearing-CMA3-ENBD20x），银存账户 78410010170010003（Cash in Banks-ENBD CMA3）
...
- 渠道必填字段 country Code / city Code 校验（ENBD、INNER 不需要 citycode；他行 IBAN 才调用 member 查 city name）。

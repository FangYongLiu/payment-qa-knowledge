...
## 可能根因
- 渠道账户（payoutAccount）实际余额低于阈值，余额确实不足。
- 余额监控 job（balanceMonitorJob）未及时刷新，redis `_cmf:accountBalance:XXXXX` 中缓存余额未更新。
- 对方银行系统波动，余额查询/出款瞬时异常。

## 排查步骤(DB/Kibana)
1. 确认渠道是否配置 payoutAccount（router 仅在 ext 配置 payoutAccount 时进行余额校验，否则跳过）：
   ```sql
   select * from router.t_channel_ext
   where channel_code in ('ENBD201','ENBD204','ENBD205')
   and attr_key = 'payoutAccount';
   ```
2. 通过 counter（account – balance monitor 规则配置）或 redis 缓存 `_cmf:accountBalance:XXXXX`（XXXXX 为渠道账号）确认余额查询是否正常。

...
## 关键方法 / 入口
**UAT Kibana 7d INFO 观测的主要业务类**(app_id=`ifb-channel`,=实际在跑的业务操作 / 入口):
- `DefaultRemChannelFacadeImpl`×209,411 — IFB 汇款渠道门面
- `SingleDetailQueryRequester`×103,479 — 单笔明细查询
- (类名为 Dubbo/RPC 处理器/门面;次数为 7d 调用量级,反映主链路。)
## 测试要点 / 排障 / 常见问题

**UAT Kibana 7d 错误观测**(自动回归 + UAT 流量,app_id=`ifb-channel`):
- ERROR 23037 次,主要:`SingleDetailQueryRequester`×22582、`QueryBalanceRequester`×214
- WARN 15 次(均为基础设施噪声)

...
> 来源:UAT Kibana trace, last 120d 宽窗口采样(2026-06-24) + 作用说明。候选待人审。app_group=`gp104` · domain=`payment-core`。
...
- ERROR 12810 次,主要:`ReportShardingTaskJob`×4461、`FabTransactionOutputConTaskHandler`×4461
- WARN 9519 次,主要:`VcTransactionReportJob`×4704、`AbstractReportTaskHandler`×4461
- ⚠️ 频次可能含 UAT 测试数据噪声,**真坑 vs 噪声需人工确认**;高频项见域级排障对象。
- 更多 QA 校验点(怎么测 / 已知坑 / 典型故障定位)待补。
## 来源与置信
- UAT Kibana trace, last 120d 宽窗口采样(2026-06-24) + 作用说明。候选待人审。app_group=`gp104` · domain=`payment-core`。

---
id: tbl_basiscustomer_t_sys_dict_type
object_type: Table
name: 字典类型表 (t_sys_dict_type)
aliases: [t_sys_dict_type, basiscustomer.t_sys_dict_type]
domain: activation
status: active
owner: xinwei.cao
reviewer: xinwei.cao
...
## 用途
物理表 `basiscustomer.t_sys_dict_type`,主键 `dict_type_id`。字典类型表。业务语义细节**待补**(表结构来自 DDL)。
...
| `description` | varchar(255) | 字典描述 · 可空 |

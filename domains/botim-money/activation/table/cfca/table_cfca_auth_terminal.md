---
id: tbl_cfca_auth_terminal
object_type: Table
name: auth_terminal (auth_terminal)
aliases: [auth_terminal, cfca.auth_terminal]
domain: activation
status: active
owner: xinwei.cao
reviewer: xinwei.cao
...
## 用途
物理表 `cfca.auth_terminal`,主键 `id`。(DDL 未提供表注释)。业务语义细节**待补**(表结构来自 DDL)。
...
## 主键 / 索引
- 主键:`id`
- `IDX_AUTH_TERMINAL_APPID`:appid
- `IDX_AUTH_TERMINAL_DEVICEID`:device_id
- `IDX_AUTH_TERMINAL_KEYID`:keyid
- `IDX_AUTH_TERMINAL_PART_HASH`:username

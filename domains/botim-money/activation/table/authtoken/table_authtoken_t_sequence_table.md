---
id: tbl_authtoken_t_sequence_table
object_type: Table
name: 序号生成表 (t_sequence_table)
aliases: [t_sequence_table, authtoken.t_sequence_table]
domain: activation
status: active
owner: xinwei.cao
reviewer: xinwei.cao
...
## 用途
物理表 `authtoken.t_sequence_table`,主键 `sequence_name`。序号生成表。业务语义细节**待补**(表结构来自 DDL)。

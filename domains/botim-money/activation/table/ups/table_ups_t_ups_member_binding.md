---
id: tbl_ups_t_ups_member_binding
object_type: Table
name: 会员绑定记录 (t_ups_member_binding)
aliases: [t_ups_member_binding, ups.t_ups_member_binding]
domain: activation
status: active
owner: xinwei.cao
reviewer: xinwei.cao
...
# 会员绑定记录 (t_ups_member_binding)

## 用途
物理表 `ups.t_ups_member_binding`,主键 `id`。会员绑定记录。业务语义细节**待补**(表结构来自 DDL)。
...
| `login_platform` | varchar(64) | 登录来源平台. 分payby, twitter, totok |

...
tags:
- aml
- t_payment_event_wide
subdomain: aml
module: null
sensitivity: normal
name: 支付事件宽表 (t_payment_event_wide)
aliases:
- t_payment_event_wide
- aml.t_payment_event_wide
related_services: []
---

# 支付事件宽表 (t_payment_event_wide)

## 用途
物理表 `aml.t_payment_event_wide`,主键 `id`。Payment event wide table。属 AML/合规库 `aml`。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_aml_t_payment_event_wide
object_type: Table
name: Payment event wide table (t_payment_event_wide)
aliases: [t_payment_event_wide, aml.t_payment_event_wide]
domain: compliance
status: active
owner: dewen.li
reviewer: dewen.li
...
# Payment event wide table (t_payment_event_wide)

## 用途
物理表 `aml.t_payment_event_wide`,主键 `id`。Payment event wide table。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_offlinepay_t_payment_order
object_type: Table
name: offline payment order (t_payment_order)
aliases: [t_payment_order, offlinepay.t_payment_order]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# offline payment order (t_payment_order)

## 用途
物理表 `offlinepay.t_payment_order`,主键 `global_id`。offline payment order。业务语义细节**待补**(表结构来自 DDL)。
...
| `clearing_time` | timestamp(3) | clearing_time · 可空 |
| `settled_time` | timestamp(3) | settled_time · 可空 |
...
| `rrn` | varchar(128) | rrn · 可空 |
| `product_code` | varchar(32) | product_code · 可空 |
| `device_info` | varchar(64) | device_info · 可空 |

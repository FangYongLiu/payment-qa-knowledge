---
id: tbl_npss_t_compensation_event
object_type: Table
name: 补偿事件表 (t_compensation_event)
aliases: [t_compensation_event, npss.t_compensation_event]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 补偿事件表 (t_compensation_event)

## 用途
物理表 `npss.t_compensation_event`,主键 `event_id`。补偿事件表。业务语义细节**待补**(表结构来自 DDL)。

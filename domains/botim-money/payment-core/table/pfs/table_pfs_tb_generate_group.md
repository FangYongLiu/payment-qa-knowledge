---
id: tbl_pfs_tb_generate_group
object_type: Table
name: 生成集配置，稳定，100行 (tb_generate_group)
aliases: [tb_generate_group, pfs.tb_generate_group]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 生成集配置，稳定，100行 (tb_generate_group)

## 用途
物理表 `pfs.tb_generate_group`,主键 `GEN_GROUP_CODE`。生成集配置，稳定，100行。业务语义细节**待补**(表结构来自 DDL)。

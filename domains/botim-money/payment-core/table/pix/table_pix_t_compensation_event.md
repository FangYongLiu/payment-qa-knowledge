---
id: tbl_pix_t_compensation_event
object_type: Table
name: Compensation Event (t_compensation_event)
aliases: [t_compensation_event, pix.t_compensation_event]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
source_type: DB DDL
source_ref: pix schema DDL
tags: [payment-core, pix]
sensitivity: normal
related_services: []
---
...
## 用途
物理表 `pix.t_compensation_event`,主键 `event_id`。Compensation Event。业务语义细节**待补**(表结构来自 DDL)。
...
| `execute_status` | char | Execute status: I=Init，F=Fail，S=Success，E=Error |

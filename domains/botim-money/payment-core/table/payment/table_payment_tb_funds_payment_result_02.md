---
id: tbl_payment_tb_funds_payment_result_02
object_type: Table
name: 成功和失败才需要保存 (tb_funds_payment_result_02)
aliases: [tb_funds_payment_result_02, payment.tb_funds_payment_result_02]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 成功和失败才需要保存 (tb_funds_payment_result_02)

## 用途
物理表 `payment.tb_funds_payment_result_02`,主键 `REQUEST_NO`。成功和失败才需要保存。业务语义细节**待补**(表结构来自 DDL)。
...
## 主键 / 索引
- 主键:`REQUEST_NO`
- `IDX_FPR_GMTMODIFIED_02`:GMT_MODIFIED
- `IDX_FUNDS_PAY_RESULT_PFID_02`:PAYMENT_FUNDS_ID
- `IDX_PAYMENT_SEQ_NO_02`:PAYMENT_SEQ_NO

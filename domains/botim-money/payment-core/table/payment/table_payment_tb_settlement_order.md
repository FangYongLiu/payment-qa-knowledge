---
id: tbl_payment_tb_settlement_order
object_type: Table
name: 结算指令 (tb_settlement_order)
aliases: [tb_settlement_order, payment.tb_settlement_order]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 结算指令 (tb_settlement_order)

## 用途
物理表 `payment.tb_settlement_order`,主键 `SETTLEMENT_ORDER_ID`。结算指令。业务语义细节**待补**(表结构来自 DDL)。
...
| `SUB_CLEARING_CODE` | varchar(32) | 清算子编码 · 可空 |

## 主键 / 索引
- 主键:`SETTLEMENT_ORDER_ID`
- `IDX_PAYMENT_CS_ID`:CLEARING_SESSION_ID
- `IDX_SO_GMTMODIFIED`:GMT_MODIFIED
- `I_TB_SORDER_PSEQNO`:PAYMENT_SEQ_NO

---
id: tbl_payment_tb_funds_payment_result
object_type: Table
name: 成功和失败才需要保存 (tb_funds_payment_result)
aliases: [tb_funds_payment_result, payment.tb_funds_payment_result]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 成功和失败才需要保存 (tb_funds_payment_result)

## 用途
物理表 `payment.tb_funds_payment_result`,主键 `REQUEST_NO`。成功和失败才需要保存。业务语义细节**待补**(表结构来自 DDL)。
...
| `PAYMENT_FUNDS_ID` | decimal(15) | 机构收付信息ID，SEQ_INSTITUTION_PAYMENT_INFO · 可空 |
...
## 主键 / 索引
- 主键:`REQUEST_NO`
- `IDX_FPR_GMTMODIFIED`:GMT_MODIFIED
- `IDX_FUNDS_PAYMENT_RESULT_PPFID`:PAYMENT_FUNDS_ID
- `IDX_PAYMENT_SEQ_NO`:PAYMENT_SEQ_NO

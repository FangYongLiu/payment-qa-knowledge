---
id: tbl_payment_tb_generate
object_type: Table
name: 生成配置，稳定，100行 (tb_generate)
aliases: [tb_generate, payment.tb_generate]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
## 用途
物理表 `payment.tb_generate`,主键 `GEN_ID`。生成配置，稳定，100行。业务语义细节**待补**(表结构来自 DDL)。

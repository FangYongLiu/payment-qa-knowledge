---
id: tbl_payment_tb_cs_carrier
object_type: Table
name: 清结算载体 (tb_cs_carrier)
aliases: [tb_cs_carrier, payment.tb_cs_carrier]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
## 用途
物理表 `payment.tb_cs_carrier`,主键 `REQUEST_NO`。清结算载体。业务语义细节**待补**(表结构来自 DDL)。
...
| `CLEARING_SESSION_ID` | char(32) | 清算场次标识:SEQ_CLEARING_SESSION_ID · 可空 |
...
## 主键 / 索引
- 主键:`REQUEST_NO`
- `UIDX_CSC_PAY_PROCESS`:PAYMENT_SEQ_NO, PS_ID, CLEARING_CODE, REQUEST_TYPE (UNIQUE)
- `IDX_CSC_GMT_MODIFIED`:GMT_MODIFIED
- `IDX_CSC_SESSION_ID`:CLEARING_SESSION_ID

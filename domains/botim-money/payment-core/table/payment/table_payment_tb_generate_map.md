---
id: tbl_payment_tb_generate_map
object_type: Table
name: 生成匹配配置，稳定，100行 (tb_generate_map)
aliases: [tb_generate_map, payment.tb_generate_map]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
## 用途
物理表 `payment.tb_generate_map`,主键 `MAP_ID`。生成匹配配置，稳定，100行。业务语义细节**待补**(表结构来自 DDL)。

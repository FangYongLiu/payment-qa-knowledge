---
id: tbl_payment_tb_biz_config
object_type: Table
name: 业务支付配置，量级同TB_PAYMENT_ORDER (tb_biz_config)
aliases: [tb_biz_config, payment.tb_biz_config]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 业务支付配置，量级同TB_PAYMENT_ORDER (tb_biz_config)

## 用途
物理表 `payment.tb_biz_config`,主键 `CONFIG_ID`。业务支付配置，量级同TB_PAYMENT_ORDER。业务语义细节**待补**(表结构来自 DDL)。
...
## 主键 / 索引
- 主键:`CONFIG_ID`
- `UK_BIZ_CONFIG_BPSN_GENCODE`:BIZ_PAYMENT_SEQ_NO, GEN_CODE (UNIQUE)
- `IDX_BIZ_CONFIG_GMT_MOD`:GMT_MODIFY

---
id: tbl_payment_tb_biz_payment_order_08
object_type: Table
name: 业务支付订单 (tb_biz_payment_order_08)
aliases: [tb_biz_payment_order_08, payment.tb_biz_payment_order_08]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 业务支付订单 (tb_biz_payment_order_08)

## 用途
物理表 `payment.tb_biz_payment_order_08`,主键 `BIZ_PAYMENT_SEQ_NO`。业务支付订单。业务语义细节**待补**(表结构来自 DDL)。
...
- `UIDX_PAYMENT_KEY_08`:PAYMENT_ORDER_NO, PRODUCT_CODE (UNIQUE)
- `IDX_BPO_GMTMODIFIED_08`:GMT_MODIFIED
- `I_BIZ_PAYMENT_ORDER_RBPSN_08`:RELATE_BIZ_PAYMENT_SEQ_NO

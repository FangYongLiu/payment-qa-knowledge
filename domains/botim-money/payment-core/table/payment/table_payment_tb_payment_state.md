---
id: tbl_payment_tb_payment_state
object_type: Table
name: tb_payment_state (tb_payment_state)
aliases: [tb_payment_state, payment.tb_payment_state]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# tb_payment_state (tb_payment_state)

## 用途
物理表 `payment.tb_payment_state`,主键 `PAYMENT_SEQ_NO`。(DDL 未提供表注释)。业务语义细节**待补**(表结构来自 DDL)。
...
## 主键 / 索引
- 主键:`PAYMENT_SEQ_NO`
- `IDX_PS_GMTMODIFIED`:GMT_MODIFIED

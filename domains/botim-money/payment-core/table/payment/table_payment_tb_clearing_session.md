---
id: tbl_payment_tb_clearing_session
object_type: Table
name: 清算场次 (tb_clearing_session)
aliases: [tb_clearing_session, payment.tb_clearing_session]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 清算场次 (tb_clearing_session)

## 用途
物理表 `payment.tb_clearing_session`,主键 `CLEARING_SESSION_ID`。清算场次。业务语义细节**待补**(表结构来自 DDL)。
...
- `IDX_CS_BATCH_NO`:BATCH_NO
- `IDX_CS_GMT_BOOKING_SHOW`:GMT_BOOKING_SHOW

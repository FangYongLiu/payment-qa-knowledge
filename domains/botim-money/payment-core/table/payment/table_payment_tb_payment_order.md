---
id: tbl_payment_tb_payment_order
object_type: Table
name: 支付指令 (tb_payment_order)
aliases: [tb_payment_order, payment.tb_payment_order]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 支付指令 (tb_payment_order)

## 用途
物理表 `payment.tb_payment_order`,主键 `PAYMENT_SEQ_NO`。支付指令。业务语义细节**待补**(表结构来自 DDL)。
...
| `PRODUCT_ORDER_NO` | varchar(64) | 产品订单号，产品体系内生成 · 可空 |
...
## 主键 / 索引
- 主键:`PAYMENT_SEQ_NO`
- `IDX_PAYMENT_ORDER_BPSN`:BIZ_PAYMENT_SEQ_NO
- `IDX_PAYMENT_ORDER_CREATE`:GMT_CREATE
- `IDX_PAYMENT_ORDER_MODIFIED`:GMT_MODIFIED
- `IDX_PAYMENT_ORDER_PO`:PAYMENT_ORDER_NO
- `IDX_PAYMENT_ORDER_RPSN`:RELATE_PAYMENT_SEQ_NO
- `I_PRODUCT_ORDER_NO`:PRODUCT_ORDER_NO

---
id: tbl_payment_tb_message
object_type: Table
name: 增长，10000行/天，至少保留三个月 (tb_message)
aliases: [tb_message, payment.tb_message]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 增长，10000行/天，至少保留三个月 (tb_message)

## 用途
物理表 `payment.tb_message`,主键 `MESSAGE_ID`。增长，10000行/天，至少保留三个月。业务语义细节**待补**(表结构来自 DDL)。
...
## 主键 / 索引
- 主键:`MESSAGE_ID`
- `UK_MESSAGE_MESSAGEKEY`:MESSAGE_KEY (UNIQUE)
- `I_MESSAGE_GMTALLOW`:GMT_ALLOW
- `I_MESSAGE_GMTMODIFY`:GMT_MODIFY
- `I_MESSAGE_MARKER`:MARKER
- `I_MESSAGE_STATUS`:NOTIFY_STATUS

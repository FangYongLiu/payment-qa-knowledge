---
id: tbl_payment_tb_biz_payment_order
object_type: Table
name: 业务支付订单 (tb_biz_payment_order)
aliases: [tb_biz_payment_order, payment.tb_biz_payment_order]
domain: payment-core
status: active
owner: xiaoyan.zhou
...
# 业务支付订单 (tb_biz_payment_order)

## 用途
物理表 `payment.tb_biz_payment_order`,主键 `BIZ_PAYMENT_SEQ_NO`。业务支付订单。业务语义细节**待补**(表结构来自 DDL)。
...
- `UIDX_PAYMENT_KEY`:PAYMENT_ORDER_NO, PRODUCT_CODE (UNIQUE)
- `IDX_BPO_GMTMODIFIED`:GMT_MODIFIED
- `I_BIZ_PAYMENT_ORDER_RBPSN`:RELATE_BIZ_PAYMENT_SEQ_NO

---
id: tbl_marketorder_t_compensation_event
object_type: Table
name: 补偿事件 (t_compensation_event)
aliases: [t_compensation_event, marketorder.t_compensation_event]
domain: marketing
status: active
owner: wujiang.ding
...
## 用途
物理表 `marketorder.t_compensation_event`,主键 `event_id`。补偿事件。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_ccctron_t_collect_currency
object_type: Table
name: 归集资产(分区) (t_collect_currency)
aliases: [t_collect_currency, ccctron.t_collect_currency]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `ccctron.t_collect_currency`,主键 `id`。归集资产(分区)。业务语义细节**待补**(表结构来自 DDL)。

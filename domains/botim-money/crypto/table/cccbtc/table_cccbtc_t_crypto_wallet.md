---
id: tbl_cccbtc_t_crypto_wallet
object_type: Table
name: 数字钱包(分区,根据address hash) (t_crypto_wallet)
aliases: [t_crypto_wallet, cccbtc.t_crypto_wallet]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `cccbtc.t_crypto_wallet`,主键 `id`。数字钱包(分区,根据address hash)。业务语义细节**待补**(表结构来自 DDL)。

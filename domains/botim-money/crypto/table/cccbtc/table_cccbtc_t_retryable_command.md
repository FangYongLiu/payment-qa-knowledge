---
id: tbl_cccbtc_t_retryable_command
object_type: Table
name: 可重试指令(分区) (t_retryable_command)
aliases: [t_retryable_command, cccbtc.t_retryable_command]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `cccbtc.t_retryable_command`,主键 `id`。可重试指令(分区)。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_ccdeposit_t_netting_order
object_type: Table
name: 入库销帐订单 (t_netting_order)
aliases: [t_netting_order, ccdeposit.t_netting_order]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
# 入库销帐订单 (t_netting_order)

## 用途
物理表 `ccdeposit.t_netting_order`,主键 `id`。入库销帐订单。业务语义细节**待补**(表结构来自 DDL)。

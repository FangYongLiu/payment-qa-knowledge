---
id: tbl_ccdeposit_t_deposit_order
object_type: Table
name: 充值订单 (t_deposit_order)
aliases: [t_deposit_order, ccdeposit.t_deposit_order]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
# 充值订单 (t_deposit_order)

## 用途
物理表 `ccdeposit.t_deposit_order`,主键 `id`。充值订单。业务语义细节**待补**(表结构来自 DDL)。
...
| `creation_type` | varchar(32) | 创建类型 MANUAL WALLET |
| `txid` | varchar(200) | txid · 可空 |

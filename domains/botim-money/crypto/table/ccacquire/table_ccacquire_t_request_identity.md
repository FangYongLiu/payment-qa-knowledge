---
id: tbl_ccacquire_t_request_identity
object_type: Table
name: 请求标识 (t_request_identity)
aliases: [t_request_identity, ccacquire.t_request_identity]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `ccacquire.t_request_identity`,主键 `global_id`。请求标识。业务语义细节**待补**(表结构来自 DDL)。

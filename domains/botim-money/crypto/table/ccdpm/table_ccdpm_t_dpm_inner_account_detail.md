---
id: tbl_ccdpm_t_dpm_inner_account_detail
object_type: Table
name: 内部账户余额明细 (t_dpm_inner_account_detail)
aliases: [t_dpm_inner_account_detail, ccdpm.t_dpm_inner_account_detail]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `ccdpm.t_dpm_inner_account_detail`,主键 `txn_seq_no`。内部账户余额明细。业务语义细节**待补**(表结构来自 DDL)。
...
| `product_code` | varchar(12) | 支付产品码 · 可空 |
| `pay_code` | varchar(12) | 支付编码 · 可空 |
| `operation_type` | decimal(1) | 交易类型 · 可空 |
...
| `biz_no` | varchar(32) | 业务号 · 可空 |
| `sub_account_after_amt` | decimal(19, 8) | 子账户后余额 · 可空 |

---
id: tbl_cctransfer_t_transfer_order
object_type: Table
name: 转账订单 (t_transfer_order)
aliases: [t_transfer_order, cctransfer.t_transfer_order]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `cctransfer.t_transfer_order`,主键 `id`。转账订单。业务语义细节**待补**(表结构来自 DDL)。

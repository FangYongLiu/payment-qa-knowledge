---
id: tbl_cctrade_t_settlement_refund_order
object_type: Table
name: 退结算订单 (t_settlement_refund_order)
aliases: [t_settlement_refund_order, cctrade.t_settlement_refund_order]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
# 退结算订单 (t_settlement_refund_order)

## 用途
物理表 `cctrade.t_settlement_refund_order`,主键 `id`。退结算订单。业务语义细节**待补**(表结构来自 DDL)。
...
| `refund_settled` | varchar(32) | 是否退结算 |
...
## 主键 / 索引
- 主键:`id`
- `i_sro_ct`:created_time
- `i_sro_lut`:last_updated_time
- `i_sro_trd`:refund_order_id

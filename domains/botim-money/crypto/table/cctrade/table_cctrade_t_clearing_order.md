---
id: tbl_cctrade_t_clearing_order
object_type: Table
name: 清分订单 (t_clearing_order)
aliases: [t_clearing_order, cctrade.t_clearing_order]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
# 清分订单 (t_clearing_order)

## 用途
物理表 `cctrade.t_clearing_order`,主键 `id`。清分订单。业务语义细节**待补**(表结构来自 DDL)。
...
## 主键 / 索引
- 主键:`id`
- `uk_co_trd`:trade_order_id (UNIQUE)

---
id: tbl_cctrade_t_atomic_payment_order
object_type: Table
name: 原子支付订单 (t_atomic_payment_order)
aliases: [t_atomic_payment_order, cctrade.t_atomic_payment_order]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
# 原子支付订单 (t_atomic_payment_order)

## 用途
物理表 `cctrade.t_atomic_payment_order`,主键 `id`。原子支付订单。业务语义细节**待补**(表结构来自 DDL)。

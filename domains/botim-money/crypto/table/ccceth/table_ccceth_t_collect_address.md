---
id: tbl_ccceth_t_collect_address
object_type: Table
name: 归集地址(分区) (t_collect_address)
aliases: [t_collect_address, ccceth.t_collect_address]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `ccceth.t_collect_address`,主键 `id`。归集地址(分区)。业务语义细节**待补**(表结构来自 DDL)。
...
| `fuel_amount` | decimal(23, 8) | 加油值 · 可空 |

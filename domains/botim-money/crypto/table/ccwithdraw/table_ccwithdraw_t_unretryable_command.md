---
id: tbl_ccwithdraw_t_unretryable_command
object_type: Table
name: 不可重试指令(分区) (t_unretryable_command)
aliases: [t_unretryable_command, ccwithdraw.t_unretryable_command]
domain: crypto
status: active
owner: can.wang
reviewer: can.wang
...
## 用途
物理表 `ccwithdraw.t_unretryable_command`,主键 `id`。不可重试指令(分区)。业务语义细节**待补**(表结构来自 DDL)。

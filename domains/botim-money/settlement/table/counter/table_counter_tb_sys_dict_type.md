---
id: tbl_counter_tb_sys_dict_type
object_type: Table
name: 字典类型表 (tb_sys_dict_type)
aliases: [tb_sys_dict_type, counter.tb_sys_dict_type]
domain: settlement
status: active
owner: xiaoyan.zhou
...
## 用途
物理表 `counter.tb_sys_dict_type`,主键 `dict_type_id`。字典类型表。业务语义细节**待补**(表结构来自 DDL)。

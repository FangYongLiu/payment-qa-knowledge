---
id: tbl_bps_t_payroll_source_bulk
object_type: Table
name: Payroll original information batch form (t_payroll_source_bulk)
aliases: [t_payroll_source_bulk, bps.t_payroll_source_bulk]
domain: wps
status: active
owner: xinwei.cao
reviewer: xinwei.cao
...
# Payroll original information batch form (t_payroll_source_bulk)

## 用途
物理表 `bps.t_payroll_source_bulk`,主键 `id`。Payroll original information batch form。业务语义细节**待补**(表结构来自 DDL)。
...
| `total_amount` | decimal(11, 2) | total amount |
| `total_quantity` | int | total quantity · 可空 |
| `submit_quantity` | int | submit quantity · 可空 |
...
| `payroll_year` | char(4) | payroll year · 可空 |
| `payroll_month` | varchar(16) | payroll month · 可空 |
...
| `distribution_type` | varchar(32) | distribution type |

...
object_type: Table
name: Unique Index for Bank Order No (t_enbd_transaction)
...
# Unique Index for Bank Order No (t_enbd_transaction)

## 用途
物理表 `vis.t_enbd_transaction`,主键 `transaction_id`。Unique Index for Bank Order No。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_acquireii_t_fiserv_sale_order
object_type: Table
name: Fiserv Sale 订单表 (t_fiserv_sale_order)
aliases: [t_fiserv_sale_order, acquireii.t_fiserv_sale_order]
...
# Fiserv Sale 订单表 (t_fiserv_sale_order)

## 用途
物理表 `acquireii.t_fiserv_sale_order`,主键 `global_id`。sale订单。属收单服务 [[svc_acquireii]]。业务语义细节**待补**(表结构来自 DDL,用途需结合代码/文档补充)。
...
| `rcpt_tx_id` | varchar(50) | txid · 可空 |
| `type` | varchar(50) | type · 可空 |

## 主键 / 索引
- 主键:`global_id`
- `i_fso_cci`:current_cmd_id
- `i_fso_ct`:created_time
- `i_fso_lut`:last_updated_time
- `i_fso_rn`:request_no
- `i_fso_txid`:rcpt_tx_id

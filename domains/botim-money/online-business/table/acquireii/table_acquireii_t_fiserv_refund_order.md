---
id: tbl_acquireii_t_fiserv_refund_order
object_type: Table
name: Fiserv 退款订单表 (t_fiserv_refund_order)
aliases: [t_fiserv_refund_order, acquireii.t_fiserv_refund_order]
...
# Fiserv 退款订单表 (t_fiserv_refund_order)

## 用途
物理表 `acquireii.t_fiserv_refund_order`,主键 `global_id`。退款订单。属收单服务 [[svc_acquireii]]。业务语义细节**待补**(表结构来自 DDL,用途需结合代码/文档补充)。
...
## 主键 / 索引
- 主键:`global_id`
- `i_frfd_ct`:created_time
- `i_frfd_lut`:last_updated_time
- `i_frfd_ogi`:origin_global_id
- `i_frfd_rn`:request_no

...
object_type: Table
name: 收单扩展表 (t_acquire_extension)
...
# 收单扩展表 (t_acquire_extension)

## 用途
物理表 `acquireii.t_acquire_extension`,主键 `id`。（DDL 未提供表注释）。属收单服务 [[svc_acquireii]]。业务语义细节**待补**(表结构来自 DDL,用途需结合代码/文档补充)。

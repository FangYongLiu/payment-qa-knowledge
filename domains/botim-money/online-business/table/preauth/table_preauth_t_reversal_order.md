---
id: tbl_preauth_t_reversal_order
object_type: Table
name: reversal订单 (t_reversal_order)
aliases: [t_reversal_order, preauth.t_reversal_order]
...
# reversal订单 (t_reversal_order)

## 用途
物理表 `preauth.t_reversal_order`,主键 `global_id`。reversal订单。业务语义细节**待补**(表结构来自 DDL)。
...
| `card_info_id` | bigint | 卡信息id · 可空 |

## 主键 / 索引
- 主键:`global_id`
- `i_ro_ct`:created_time
- `i_ro_lut`:last_updated_time
- `i_ro_oid`:origin_global_id
- `i_ro_rn`:request_no

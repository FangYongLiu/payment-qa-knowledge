...
object_type: Table
name: 商户结算任务表 (t_merchant_settle_job)
...
# 商户结算任务表 (t_merchant_settle_job)

## 用途
物理表 `mchtsettle.t_merchant_settle_job`,主键 `id`。merchant settle job。属商户结算服务 [[svc_merchant_settlement]]。业务语义细节**待补**(表结构来自 DDL)。

...
# 清算批次表 (t_clearing_batch)

## 用途
物理表 `mchtsettle.t_clearing_batch`,主键 `id`。clearing batch。属商户结算服务 [[svc_merchant_settlement]]。业务语义细节**待补**(表结构来自 DDL)。

---
id: reference_merchant_acquisition_test_methodology_overview
object_type: Reference
name: 商户收单测试方法论总览
aliases: [merchant-acquisition-test-methodology-overview]
...
tags: [online-business, wiki-overview]
related_services: [svc_acquireii, svc_cmf, svc_preauth, svc_qpay_mpgs, svc_statementii, svc_tradeii]
...
本页定义商户收单测试的通用方法论,是所有场景手册(§5.x)依赖的"骨架"。先读懂本页,再去看具体场景。隶属业务域:domain_merchant_acquisition。

## 核心理念

商户收单测试**不是只测商户接口**,而是要验证整条链路:**支付中台调用链 + 数据库落库 + 资金流 + 通知 + 对账**。一笔成功的接口响应只是测试的**起点**,不是终点。
...
| 模块 | 作用 | 子页 |
|---|---|---|
| 测试环境与工具 | 域名、SGS、API Key、MySQL、测试账号、MID | merchant-acquisition-test-environment-and-tools |
...
| 测试数据准备规范 | merchantOrderNo 命名 / 金额选择 / 卡号 / 回调 | merchant-acquisition-test-data-conventions |

## 五段式 E2E 验证骨架(核心方法论)

这是本方法论最重要的一节。所有场景手册的测试用例都按以下 5 段来验证,**缺一不可**。详细展开见 merchant-acquisition-e2e-five-stage-verification。
...
| ⑤ 通知 & 对账(Notify & Statement) | 异步通知与 T+1 对账 | `notifyUrl` 触发、商户回 `{"response":"SUCCESS"}` 后不再重试、T+1 对账单 CSV 包含本笔 |
...
3. 读 merchant-acquisition-e2e-five-stage-verification 理解 5 段验证骨架(**最重要**)。
4. 读 merchant-acquisition-test-case-skeleton-abcde 和 merchant-acquisition-test-data-conventions 知道用例要覆盖哪些维度、数据怎么造。
5. 进入具体场景手册(§5.x:PAYPAGE / DIRECTPAY / PREAUTH 等),按本页 5 段骨架逐项核对。
6. 跑用例时随手翻 merchant-acquisition-error-code-quickref;每笔交易跑完,对照 merchant-acquisition-per-transaction-checklist 打勾收尾。

...
related_services: [svc_sgs, svc_fundout, svc_xbh_gateway, svc_merchant_console_frontend, svc_cash_eatm, svc_customer_frontend, svc_acquireii, svc_merchant_fundout, svc_adtaxi, svc_statementii, svc_preauth, svc_pos_exception_processor, svc_acquire2_query, svc_merchant_settlement, svc_jollychic_service]
---

# online-business(线上业务)

> 业务域总览(入口节点)。owner fangyong.liu。细节见各服务对象。
> **架构分层总览(网关/Facade/核心/商户/结算,跨域):[[reference_online_business_acquiring_architecture]]**

## 概述
线上业务域:收单(acquiring)、出款(fundout)、商户/客户前端、收银(cash-eatm)、预授权、POS 异常处理、对账单等线上经营能力。ToB 收单网关 sgs 亦属本域(ToC 网关 cgs 属 payment-core)。

## 覆盖范围
共 15 个服务:sgs(ToB 收单网关)、fundout、xbh-gateway、merchant-console-frontend、cash-eatm、customer-frontend、acquireii、merchant-fundout、adtaxi、statementii、preauth、pos-exception-processor、acquire2-query、merchant-settlement、jollychic-service。
...
- [[reference_merchant_acquisition_test_methodology_overview]](商户收单测试方法论总览)
- [[reference_payment_scenarios_overview]](支付场景现状梳理:DirectPay/PayAndSign/AutoDebit 等)
- [[reference_scan_to_pay_channels]](Scan to Pay 各渠道差异:AliPay / UnionPay / …)
- [[reference_pos_business_overview]](POS 业务总览)
- [[reference_enbd_3ds_auth_screen]](ENBD / Liv 3DS 认证页面与认证方式)
- [[reference_chargeback_email_routing]](ChargeBack 邮件通知收件人路由)

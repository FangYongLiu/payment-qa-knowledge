---
id: tbl_acquire_t_refund_order
object_type: Table
name: 收单数据 (t_refund_order)
aliases: [t_refund_order, acquire.t_refund_order]
domain: offline-business
status: active
owner: xiaoqian.wei,wanmei.ding
reviewer: xiaoqian.wei,wanmei.ding
last_reviewed_at: '2026-07-02'
source_type: DB DDL
source_ref: acquire schema DDL
tags: [offline-business, acquire]
sensitivity: normal
related_services: []
---

# 收单数据 (t_refund_order)

## 用途
物理表 `acquire.t_refund_order`,主键 `global_id`。收单数据。业务语义细节**待补**(表结构来自 DDL)。

## 关联关系
- **所属服务**:待补。
- **谁读写它**:相关服务 / 接口(由对方文档 `related_tables` 声明)。
- **哪些场景校验它**:待补。

## 关键列
| 列 | 类型 | 说明 |
| --- | --- | --- |
| `global_id` | bigint | 退款全局号 |
| `acquire_global_id` | bigint | 收单全局号 |
| `refund_amount` | decimal(19, 4) | 收单金额 |
| `notify_url` | varchar(500) | 商户通知地址 · 可空 |
| `derive_status` | varchar(200) | 待补 |
| `refund_operator_name` | varchar(200) | 待补 · 可空 |
| `created_time` | timestamp(3) | 创建时间 |
| `last_updated_time` | timestamp(3) | 最后更新时间 |
| `data_version` | bigint | 数据版本 |

## 主键 / 索引
- 主键:`global_id`
- `i_ao_ag`:acquire_global_id
- `i_ao_lut`:last_updated_time

## 校验点(QA 关注)
- **时间字段**:创建/更新时间;按时间过滤走对应索引。
- **乐观锁**:更新须带 `data_version`,并发校验版本递增。
- **金额精度**:decimal 比较用容差(< 0.01);amount 与 currency 需一致。
- 业务语义、状态枚举、跨表关联**待补**(需结合代码或业务文档)。

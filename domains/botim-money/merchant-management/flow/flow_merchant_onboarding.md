...
3. **BMOC 审核(Merchant Status Approval)** —— [[svc_basis_merchant]]。BMOC 登录 LDAP 员工域账号(basis-cas),菜单 `Basis Merchant => BUSINESS => Merchant`。
   - **Portal 自助申请**(Source=`PORTAL`)在 **`Merchant Approval`** 队列(列表列:Apply ID / Source(PORTAL/BASIS/AGENT)/ Merchant ID / Merchant Name / Contact / Application 日期 / **Status**(`Awaiting Approval`→`Approved`)/ Action)。行操作:`View`(看申请详情)/ `Approval`(审批)/ `Add Staff`;批准后行操作变为 `View / Open / Activate Alipay / Add Staff`。
   - **Approval 表单**(点 `Approval`):`Result`(**PASS**/REJECT)、Whether Internal(External)、`*Business Name`、`*Category`(MCC 类目,如 Miscellaneous General Merchandise)、Business Type(Merchant)、`*Country/Region`、`*Emirate/City`、`*Business Address`、Tax Registration Number、Compliance Level、Director/Share Holder/Beneficial Owner/Authorized Signer、`*Business Organisational Structure`(=LLC)、**`*Screening result`(制裁筛查结果,必传)**、**`*EID Front/Back Photo`(受益人 EID 正反面,必传)**、Store Type/Store Logo、`*Reason`(审核意见)→ `Confirm`。这一步即 **KYB(企业资质)+ 制裁筛查** 人审门;AML Risk Calculate 失败会阻断激活([[svc_aml]])。
   - 其它相关菜单(同 `Merchant` 下):`Merchant Onboarding Management`(渠道报备/Fund Provider,≠此处 KYB)、`Merchant Management`、`Application Approval`、`Tax Info Approval`、`Bank Account Approval`、`Common Approval`、`Merchant Compliance Audit Log`(审计日志)等。
   - `Confirm` 后 Status→`Approved`,商户激活;后续可 `Activate Alipay` 等开通渠道。

---
id: tbl_basismcht_tb_export_template
object_type: Table
name: tb_export_template (tb_export_template)
aliases: [tb_export_template, basismcht.tb_export_template]
...
## 用途
物理表 `basismcht.tb_export_template`,主键 `template_id`。(DDL 未提供表注释)。业务语义细节**待补**(表结构来自 DDL)。

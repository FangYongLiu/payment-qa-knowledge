---
id: tbl_basismcht_t_audit_record
object_type: Table
name: 审核记录 (t_audit_record)
aliases: [t_audit_record, basismcht.t_audit_record]
...
## 用途
物理表 `basismcht.t_audit_record`,主键 `audit_id`。审核记录。业务语义细节**待补**(表结构来自 DDL)。
...
| `audit_status` | varchar(32) | 审核状态：AW待申请，AF申请失败，W待审核，P通过，R拒绝 |
| `execute_status` | varchar(32) | 执行状态：P处理中，S成功，F失败 · 可空 |

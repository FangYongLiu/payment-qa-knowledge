---
id: tbl_basismcht_t_operation_user_record
object_type: Table
name: 运营用户会话记录表 (t_operation_user_record)
aliases: [t_operation_user_record, basismcht.t_operation_user_record]
...
## 用途
物理表 `basismcht.t_operation_user_record`,主键 `id`。运营用户会话记录表。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_basismcht_t_audit_log
object_type: Table
name: 审核日志 (t_audit_log)
aliases: [t_audit_log, basismcht.t_audit_log]
...
## 用途
物理表 `basismcht.t_audit_log`,主键 `log_id`。审核日志。业务语义细节**待补**(表结构来自 DDL)。
...
| `action` | varchar(32) | 操作：P通过，D拒绝，R重试 |
| `remark` | varchar(3072) | 评论 · 可空 |

---
id: tbl_cms_tb_sys_menu
object_type: Table
name: 菜单表 (tb_sys_menu)
aliases: [tb_sys_menu, cms.tb_sys_menu]
...
## 用途
物理表 `cms.tb_sys_menu`,主键 `menu_id`。菜单表。业务语义细节**待补**(表结构来自 DDL)。
...
| 列 | 类型 | 说明 |
| --- | --- | --- |
| `menu_id` | bigint(18) | 主键id · 可空 |
| `code` | varchar(255) | 菜单编号 · 可空 |
...
| `sort` | tinyint | 菜单排序号 · 可空 |
| `levels` | tinyint | 菜单层级 · 可空 |
| `menu_flag` | varchar(32) | 是否是菜单(字典) · 可空 |

...
- **审核激活**:BMOC `Basis Merchant => Business => Merchant => Merchant Onboarding`(AML Risk Calculate + KYB Approval),通过后 `t_merchant.Status=MERCHANT_ENABLED`、`tm_member` 状态=1(见 [[flow_merchant_onboarding]] / [[reference_bmoc_basis_merchant]])。

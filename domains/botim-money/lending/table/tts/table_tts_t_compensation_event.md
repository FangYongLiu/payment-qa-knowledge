---
id: tbl_tts_t_compensation_event
object_type: Table
name: Compensation events table (t_compensation_event)
aliases: [t_compensation_event, tts.t_compensation_event]
domain: lending
status: active
owner: Xiaopei.Yan
...
## 用途
物理表 `tts.t_compensation_event`,主键 `event_id`。Compensation events table。业务语义细节**待补**(表结构来自 DDL)。
...
| `allow_time` | timestamp(3) | Allowed execution time · 可空 |
| `create_time` | timestamp(3) | Creation time |

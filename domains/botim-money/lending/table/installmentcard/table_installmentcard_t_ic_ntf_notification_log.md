...
object_type: Table
name: Customer notification audit log — one row per (notification x channel) attempt (t_ic_ntf_notification_log)
...
# Customer notification audit log — one row per (notification x channel) attempt (t_ic_ntf_notification_log)

## 用途
物理表 `installmentcard.t_ic_ntf_notification_log`,主键 `id`。Customer notification audit log — one row per (notification x channel) attempt。业务语义细节**待补**(表结构来自 DDL)。
...
| `variant` | varchar(32) | Free-form discriminator when (type, ref_type, ref_id) alone is not unique. Reminders use day offset (e.g., "5", "30"); other types leave it NULL · 可空 |
| `channel` | tinyint | Delivery channel as integer enum (TINYINT). Example mapping: 1=OA, 2=SMS, 3=EMAIL |
| `created_at` | datetime | When the claim row was inserted (send attempt started) |
| `sent_at` | datetime | When the underlying channel send was confirmed successful. NULL indicates the send was not confirmed (failed, crashed mid-send, or still in flight) · 可空 |

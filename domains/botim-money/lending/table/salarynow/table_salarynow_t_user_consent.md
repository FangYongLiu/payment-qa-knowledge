---
id: tbl_salarynow_t_user_consent
object_type: Table
name: Stores user consent for user (t_user_consent)
aliases: [t_user_consent, salarynow.t_user_consent]
domain: lending
status: active
owner: Xiaopei.Yan
...
## 用途
物理表 `salarynow.t_user_consent`,主键 `id`。Stores user consent for user。业务语义细节**待补**(表结构来自 DDL)。
...
| 列 | 类型 | 说明 |
| --- | --- | --- |
| `id` | bigint | Unique ID for each consent record · 可空 |
| `member_id` | varchar(20) | Reference to the user who gave consent |
| `application_id` | bigint | application for which consent recorded if any · 可空 |
...
## 主键 / 索引
- 主键:`id`
- `idx_t_user_consent_application_id`:application_id
- `idx_t_user_consent_member_id`:member_id

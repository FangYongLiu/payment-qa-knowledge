---
id: tbl_instalment_t_ci_repay_split
object_type: Table
name: 还款分帐 (t_ci_repay_split)
aliases: [t_ci_repay_split, instalment.t_ci_repay_split]
domain: lending
status: active
owner: Xiaopei.Yan
...
# 还款分帐 (t_ci_repay_split)

## 用途
物理表 `instalment.t_ci_repay_split`,主键 `id`。还款分帐。业务语义细节**待补**(表结构来自 DDL)。

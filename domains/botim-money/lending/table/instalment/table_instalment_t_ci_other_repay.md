---
id: tbl_instalment_t_ci_other_repay
object_type: Table
name: 其它还款类型记录 (t_ci_other_repay)
aliases: [t_ci_other_repay, instalment.t_ci_other_repay]
domain: lending
status: active
owner: Xiaopei.Yan
...
# 其它还款类型记录 (t_ci_other_repay)

## 用途
物理表 `instalment.t_ci_other_repay`,主键 `id`。其它还款类型记录。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_instalment_t_ci_invoice_task
object_type: Table
name: 创建发票记录表，用于失败补偿 (t_ci_invoice_task)
aliases: [t_ci_invoice_task, instalment.t_ci_invoice_task]
domain: lending
status: active
owner: Xiaopei.Yan
...
# 创建发票记录表，用于失败补偿 (t_ci_invoice_task)

## 用途
物理表 `instalment.t_ci_invoice_task`,主键 `id`。创建发票记录表，用于失败补偿。业务语义细节**待补**(表结构来自 DDL)。
...
| `type` | smallint(4) | 待补 |
| `detail` | varchar(255) | 调用发票接口用到的参数，json格式 · 可空 |

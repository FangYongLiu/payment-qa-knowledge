---
id: tbl_botimsnpl_t_sl_bill_split
object_type: Table
name: 还款分帐 (t_sl_bill_split)
aliases: [t_sl_bill_split, botimsnpl.t_sl_bill_split]
domain: lending
status: active
owner: Xiaopei.Yan
...
# 还款分帐 (t_sl_bill_split)

## 用途
物理表 `botimsnpl.t_sl_bill_split`,主键 `id`。还款分帐。业务语义细节**待补**(表结构来自 DDL)。

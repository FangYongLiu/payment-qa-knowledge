---
id: tbl_botimsnpl_t_sl_repay_record
object_type: Table
name: 还款记录 (t_sl_repay_record)
aliases: [t_sl_repay_record, botimsnpl.t_sl_repay_record]
domain: lending
status: active
owner: Xiaopei.Yan
...
# 还款记录 (t_sl_repay_record)

## 用途
物理表 `botimsnpl.t_sl_repay_record`,主键 `id`。还款记录。业务语义细节**待补**(表结构来自 DDL)。
...
- `idx_repay_record_refno`:referenceNo
- `idx_repay_record_repay_no`:repay_no
- `idx_repay_record_userid`:user_id

---
id: tbl_appscore_t_compensation_event
object_type: Table
name: 补偿事件 (t_compensation_event)
aliases: [t_compensation_event, appscore.t_compensation_event]
domain: lending
status: active
owner: Xiaopei.Yan
...
## 用途
物理表 `appscore.t_compensation_event`,主键 `event_id`。补偿事件。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_appscore_t_points_transfer
object_type: Table
name: 积分交易订单 (t_points_transfer)
aliases: [t_points_transfer, appscore.t_points_transfer]
domain: lending
status: active
owner: Xiaopei.Yan
...
# 积分交易订单 (t_points_transfer)

## 用途
物理表 `appscore.t_points_transfer`,主键 `points_transfer_no`。积分交易订单。业务语义细节**待补**(表结构来自 DDL)。
...
| 列 | 类型 | 说明 |
| --- | --- | --- |
| `points_transfer_no` | bigint(18) | 主键 |
| `merchant_order_no` | varchar(64) | 商户订单号 |
| `order_no` | varchar(50) | 订单号 |
| `biz_product_code` | varchar(15) | 业务产品码 |
| `cleaning_code` | varchar(20) | 清算编码 · 可空 |
| `payer_mid` | varchar(32) | 收款方会员id |
| `payee_mid` | varchar(32) | 付款方会员id |
| `points` | decimal(15, 2) | 积分数量 |
| `status` | varchar(15) | 状态 |
| `request_time` | timestamp | 请求时间 · 可空 |
| `notify_url` | varchar(255) | 通知路径 · 可空 |

---
id: tbl_blackcredit_t_unretryable_command
object_type: Table
name: 不可重试指令(分区) (t_unretryable_command)
aliases: [t_unretryable_command, blackcredit.t_unretryable_command]
domain: lending
status: active
owner: Xiaopei.Yan
...
## 用途
物理表 `blackcredit.t_unretryable_command`,主键 `id`。不可重试指令(分区)。业务语义细节**待补**(表结构来自 DDL)。

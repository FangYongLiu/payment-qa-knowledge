---
id: tbl_botimcredit_t_cb_credit_snapshot
object_type: Table
name: 用户额度快照 (t_cb_credit_snapshot)
aliases: [t_cb_credit_snapshot, botimcredit.t_cb_credit_snapshot]
domain: lending
status: active
owner: Xiaopei.Yan
...
# 用户额度快照 (t_cb_credit_snapshot)

## 用途
物理表 `botimcredit.t_cb_credit_snapshot`,主键 `id`。用户额度快照。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_botimcredit_t_cb_push_config
object_type: Table
name: Scheduled push configuration table (t_cb_push_config)
aliases: [t_cb_push_config, botimcredit.t_cb_push_config]
domain: lending
status: active
owner: Xiaopei.Yan
...
# Scheduled push configuration table (t_cb_push_config)

## 用途
物理表 `botimcredit.t_cb_push_config`,主键 `id`。Scheduled push configuration table。业务语义细节**待补**(表结构来自 DDL)。
...
| `msg_content` | varchar(255) | SMS message content · 可空 |

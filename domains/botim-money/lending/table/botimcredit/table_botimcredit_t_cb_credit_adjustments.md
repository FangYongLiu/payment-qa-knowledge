---
id: tbl_botimcredit_t_cb_credit_adjustments
object_type: Table
name: credit limit adjustment table (t_cb_credit_adjustments)
aliases: [t_cb_credit_adjustments, botimcredit.t_cb_credit_adjustments]
domain: lending
status: active
owner: Xiaopei.Yan
...
# credit limit adjustment table (t_cb_credit_adjustments)

## 用途
物理表 `botimcredit.t_cb_credit_adjustments`,主键 `id`。credit limit adjustment table。业务语义细节**待补**(表结构来自 DDL)。
...
| `application_id` | varchar(45) | risk application id · 可空 |
| `source_type` | tinyint | 1-easycash, 2-risk · 可空 |

## 主键 / 索引
- 主键:`id`
- `idx_adj_application_id`:application_id

---
id: tbl_botimcredit_t_cb_order
object_type: Table
name: 订单表 (t_cb_order)
aliases: [t_cb_order, botimcredit.t_cb_order]
domain: lending
status: active
owner: Xiaopei.Yan
...
## 用途
物理表 `botimcredit.t_cb_order`,主键 `id`。订单表。业务语义细节**待补**(表结构来自 DDL)。
...
| `gid` | varchar(50) | trade request no · 可空 |

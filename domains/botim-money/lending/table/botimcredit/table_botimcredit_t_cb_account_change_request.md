---
id: tbl_botimcredit_t_cb_account_change_request
object_type: Table
name: easycash account change request table (t_cb_account_change_request)
aliases: [t_cb_account_change_request, botimcredit.t_cb_account_change_request]
domain: lending
status: active
owner: Xiaopei.Yan
...
# easycash account change request table (t_cb_account_change_request)

## 用途
物理表 `botimcredit.t_cb_account_change_request`,主键 `id`。easycash account change request table。业务语义细节**待补**(表结构来自 DDL)。

---
id: tbl_credit_t_cs_credit
object_type: Table
name: 授信表 (t_cs_credit)
aliases: [t_cs_credit, credit.t_cs_credit]
domain: lending
status: active
owner: Xiaopei.Yan
...
## 用途
物理表 `credit.t_cs_credit`,主键 `id`。授信表。业务语义细节**待补**(表结构来自 DDL)。
...
| `is_read` | smallint(2) | 是否已读: 0 未读  1已读 · 可空 |
| `type` | smallint(2) | 类型 1授信 2订单 |
| `is_pass_general` | smallint(2) | 是否展示单期 |

...
# 内容 (t_partner_notify_data)

## 用途
物理表 `pns.t_partner_notify_data`,主键 `NOTIFY_DATA_ID`。内容。业务语义细节**待补**(表结构来自 DDL)。

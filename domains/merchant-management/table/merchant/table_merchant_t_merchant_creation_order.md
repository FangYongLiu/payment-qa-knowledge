---
id: tbl_merchant_t_merchant_creation_order
object_type: Table
...
module: null
sensitivity: normal
name: 商户创建订单(t_merchant_creation_order)
aliases:
- t_merchant_creation_order
related_services:
- svc_merchant
related_scenarios: []
---
# 商户创建订单(t_merchant_creation_order)

## 用途
商户创建订单。属 merchant 库,由 [[svc_merchant]] 读写。
...
| `ext_apply_id` | varchar(32) | NOT NULL | 外部申请id |
| `name` | varchar(100) | NOT NULL | 商户简称 |
| `status` | varchar(32) | NOT NULL | 状态 CREATED 已创建 PASSED 已通过 REJECTED 已拒绝 |
...
| `applier_mid` | bigint | NOT NULL | 申请人会员id |
...
| `id_front_photo_path` | varchar(200) |  | id卡正面路径 |
| `id_back_photo_path` | varchar(200) |  | id卡背面路径 |
...
| `create_source` | varchar(50) |  | 创建来源 PORTAL 商户控台  AGENT  代理商 |
| `agent_id` | bigint |  | 代理数据ID |
...
| `industry_type` | varchar(50) |  | Industry Type(行业类型) |
...
| `is_pay_facs` | tinyint(1) | NOT NULL / 默认 0 | Indicates if the creation order involves a PayFac |
...
- 主键:(`id`)
- 唯一约束:
  - `uk_ext_apply_id` 唯一 (ext_apply_id)
- 索引:

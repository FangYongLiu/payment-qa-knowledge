...
## 用途
- 处理非 lean 场景下 fab 用户的 IBAN 申请
- fab 用户在 payby app 充值入口(Bank Account Transfer)点击激活后,申请记录落库到 vis.t_virtual_account(通过 mid 关联),初始 status=Initial
- 后续由该批次 job 统一处理:job 触发后批量调用 fab,更新 IBAN 数据状态为 Valid
...
- 申请落库表:vis.t_virtual_account(通过 mid 关联),初始 status=Initial,job 处理后更新为 Valid
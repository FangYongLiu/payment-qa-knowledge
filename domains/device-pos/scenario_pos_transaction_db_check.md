...
- **终端/设备**:`tbl_acquire_terminal` / `t_terminal_detail` / `tbl_merchant_device`(商户-设备绑定)/ `tbl_hardware_info` / `tbl_active_code`(激活码)/ `tbl_device_apply_order`(申请单)
- **凭证/渠道**:`tbl_pcc_content`(支付授权凭证)/ `t_fiserv_channel_param`(渠道参数)
- **关联收单主链路**:`t_acquire_order` / `t_payment_info`

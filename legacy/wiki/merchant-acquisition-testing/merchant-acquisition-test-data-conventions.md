...
source_ref: bimoqa-conversation:3253a71ea959
tags: []
---

# 商户收单测试数据准备规范

本页定义商户收单测试中通用的测试数据约定：订单号命名、金额选择、测试卡号、特殊 expiry 触发、SIM 环境 OTP、notifyUrl mock 工具。所有场景手册的用例数据都应遵循本页规范。相关方法论参见 [[merchant-acquisition-test-methodology-overview]]，环境与账号信息见 [[merchant-acquisition-test-environment-and-tools]]。
...
在 SIM（模拟）环境下，3DS / OTP 验证页面的默认 OTP 通常为以下两者之一：

- `123456`
- `132580`

如果其中一个不通过，换另一个再试。生产环境不适用。
